...
for base in ${bases[@]}
do
    outp_file="rgcn_aifb_base_$base.out"
    for i in {1..10}
    do
        python entity_classify.py -d aifb --testing --gpu 0 --n-epochs 100 -r $i --n-bases $base  >> $outp_file
    done
done
...
for rank in ${ranks[@]}
do
    outp_file="rgcn_aifb_tucker_rank_$base.out"
    for i in {1..10}
    do
...
for emb in ${embs[@]}
do
    outp_file="rgcn_aifb_emb_$emb.out"
    for i in {1..10}
    do
        python entity_classify.py -d aifb --testing --gpu 0 --n-epochs 100 -r $i --embedding --n-hidden $emb >> $outp_file
    done
done
