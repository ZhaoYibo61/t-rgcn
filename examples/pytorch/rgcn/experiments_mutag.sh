...
for base in ${bases[@]}
do
    outp_file="rgcn_mutag_base_$base.out"
    for i in {1..10}
    do
        python entity_classify.py -d mutag --testing --gpu 0 --n-epochs 100 -r $i --n-bases $base  >> $outp_file
    done
done
...
for emb in ${embs[@]}
do
    outp_file="rgcn_mutag_emb_$emb.out"
    for i in {1..10}
    do
...
for rank in ${ranks[@]}
do
    outp_file="rgcn_mutag_tucker_rank_$base.out"
    for i in {1..10}
    do
